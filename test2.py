...
class TrainManager:
    ...
    def train_model(self, model: VPIDQN2, steps_amount: int) -> None:
    ...
    def vpi_train_model(self, model: VPIDQN2, steps_amount: int) -> None:
        for t in range(steps_amount):
            print(f"{100 * t / (steps_amount - 1):.2f}%", end="\r")
...
for t in range(10):
    ...
    for i in range(normal_training_epochs, normal_training_epochs + vpi_training_epochs):
        ...
        start = time.perf_counter()
        train_manager.vpi_train_model(model, 10000)
        end = time.perf_counter()
        print(f"{end - start:.3f}s")
