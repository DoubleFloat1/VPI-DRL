...
class ExperienceManagerInterface:
    def add_experience(self, state: Tensor, action: int | ndarray, reward: float, next_state: Tensor, episode_terminated: bool, priority: float = 1.0) -> None:
        raise NotImplementedError
    
    def can_get_batch(self) -> bool:
        raise NotImplementedError
    
    def get_batch(self, discrete_action: bool = True) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        raise NotImplementedError
...
class ExperienceManager(ExperienceManagerInterface):
    def __init__(self, max_size: int, batch_size: int, state_size: List[int], state_to_uint8: bool, device: torch.device, action_size: int = 1):
        ...
        self.action_batch: Tensor = torch.zeros(max_size, action_size, dtype=torch.float32).to(device)
    ...
    def add_experience(self, state: Tensor, action: int | ndarray, reward: float, next_state: Tensor, episode_terminated: bool, priority: float = 1.0) -> None:
        ...
        if isinstance(action, int):
            self.action_batch[self.next_index] = torch.tensor([action], dtype=torch.float32).to(self.device)
        else:
            self.action_batch[self.next_index] = torch.tensor(action, dtype=torch.float32).to(self.device)
    ...
    def empty_experience(self) -> None:
        ...
        self.action_batch = torch.zeros(self.max_size, self.action_size, dtype=torch.float32).to(self.device)
...
class PrioritizedExperienceManager(ExperienceManagerInterface):
    def __init__(self, state_size: List[int], params: VPIDQNParams, device: torch.device, action_size: int = 1):
        self.params: VPIDQNParams = params
        self.device: torch.device = device
        self.state_size: List[int] = state_size
        self.action_size: int = action_size
        
        self.state_batch_dimensions: List[int] = [params.experience_replay_max_size] + state_size
        self.state_dtype: torch.dtype = torch.uint8 if params.experience_replay_state_to_uint8 else torch.float32

        self.state_batch: Tensor = torch.zeros(self.state_batch_dimensions, dtype=self.state_dtype).to(device)
        self.action_batch: Tensor = torch.zeros(params.experience_replay_max_size, self.action_size, dtype=torch.float32).to(device)
        self.reward_batch: Tensor = torch.zeros(params.experience_replay_max_size, 1, dtype=torch.float32).to(device)
        self.next_state_batch: Tensor = torch.zeros(self.state_batch_dimensions, dtype=self.state_dtype).to(device)
    ...
    def add_experience(self, state: Tensor, action: int | Tensor, reward: float, next_state: Tensor, episode_terminated: bool, priority: float = 1.0) -> None:
        exp_index: int = self.priority.add_new_priority(priority**self.params.alpha)
        self._add_experience(exp_index, state, action, reward, next_state, episode_terminated)
    
    def _add_experience(self, exp_index: int, state: Tensor, action: int | ndarray, reward: float, next_state: Tensor, episode_terminated: bool) -> None:
        if self.state_dtype == torch.uint8:
            state = state.detach().clone().mul(255.0).to(torch.uint8)
            next_state = next_state.detach().clone().mul(255.0).to(torch.uint8)
        ...
        if exp_index != -1:
            ...
            if isinstance(action, int):
                self.action_batch[exp_index] = torch.tensor([action], dtype=torch.float32).to(self.device)
            else:
                self.action_batch[exp_index] = torch.tensor(action, dtype=torch.float32).to(self.device)
    ...
    def update_priorities(self, priorities: Tensor) -> None:
        self.priority.update_last_batch_priorities(priorities**self.params.alpha)
