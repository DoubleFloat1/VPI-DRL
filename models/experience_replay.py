...
class PrioritizedExperienceManager(ExperienceManagerInterface):
    def __init__(self, state_size: List[int], params: VPIDQNParams, device: torch.device):
        ...
        self.state_batch: Tensor = torch.zeros(self.state_batch_dimensions, dtype=self.state_dtype).to(device)
        self.action_batch: Tensor = torch.zeros(params.experience_replay_max_size, 1, dtype=torch.int32).to(device)
